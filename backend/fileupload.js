...
const FileSchema = new mongoose.Schema({
  ...
  fileType: [
    { type: mongoose.Schema.Types.ObjectId, ref: "FileType", required: true },
  ],
...
router.delete("/deletefile/:fileId", async (req, res) => {
  const { fileId } = req.params; // Use fileId from the route parameter

  try {
    const fileToDelete = await File.findById(fileId); // Use findById to find the file by its ID
    if (!fileToDelete) {
      return res.status(404).json({ error: "File not found" });
    }

    const filePath = path.join(__dirname, "uploads", fileToDelete.fileName); // Path to the file
    await fs.promises.unlink(filePath); // Delete the file from the server

    await File.deleteOne({ _id: fileId }); // Delete the file record from the database by ID
...
router.get("/download/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to download file", details: error.message });
  }
});
